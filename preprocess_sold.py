...
class NumericalTransformer(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X):
        # print(X.head(10))
        if self.dropna:
            X = X.drop(['dereg_value', 'depreciation', 'mileage'], axis=1) # too many na
        # X = X.replace(r'\d+', np.NaN, regex=True)
        cols = list(X)
        for col in cols:
            X[col] = X[col].astype(str).str.replace('', '0')
            X[col] = X[col].str.extract('(^\d*)').astype(int)
...
class LabelCategoricalTransformer(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X):
        ...
        X = X.drop(['listing_id', 'opc_scheme'], axis=1) # rm unique and redundant cols
...
class PreprocessedDataFrame():
    def __init__(self, X_train, y_train, target_encoding=False):
        ...
        self.cat_attribs = ['listing_id', 'title', 'make', 'model', 'type_of_vehicle', 'category', 'transmission', 'fuel_type', 'features', 'accessories', 'description', 'opc_scheme']
    ...
    def print_attribs_type(self):
        ...
        print(len(self.date_attribs) + len(self.num_attribs) + len(self.oh_cat_attribs) + len(self.label_cat_attribs))
        print()
    ...
    def build_dataframe(self):
        ...
        print('transformed_attribs', len(transformed_attribs), transformed_attribs)
